...
_LOGGER_NAME = "learning"
_configured = False


def get_learning_logger() -> logging.Logger:
    global _configured
    ...
    log_path = Path(LOG_DIR) / "learning.log"
    ...
    try:
        ...
        legacy_path = legacy_dir / "learning.log"
...
def log_learning_event(event: str, data: Dict[str, Any], *, error: Optional[str] = None) -> None:
    payload = {"ts": time.time(), "event": event, "data": data, "error": error}
    try:
        get_learning_logger().info(json.dumps(payload, ensure_ascii=False))
    except Exception:
        return
