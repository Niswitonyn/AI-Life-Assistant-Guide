...
def get_security_logger() -> logging.Logger:
    ...
    log_path = Path(LOG_DIR) / "security.log"
    ...
    try:
        ...
        legacy_path = legacy_dir / "security.log"
...
def log_security_event(event: str, data: Dict[str, Any], *, error: Optional[str] = None) -> None:
    payload = {"ts": time.time(), "event": event, "data": data, "error": error}
    try:
        get_security_logger().info(json.dumps(payload, ensure_ascii=False))
    except Exception:
        return
