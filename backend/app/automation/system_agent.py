...
class SystemAgent:
    ...
    def system_action(self, text: str):
        ...
        if "volume up" in cmd or "increase volume" in cmd or "raise volume" in cmd:
            self._tap_key(self.VK_VOLUME_UP, times=6)
            return "Volume increased"

        if "volume down" in cmd or "decrease volume" in cmd or "lower volume" in cmd:
            self._tap_key(self.VK_VOLUME_DOWN, times=6)
        ...
        if "unmute" in cmd:
            self._tap_key(self.VK_VOLUME_MUTE, times=1)
            return "Volume toggled (unmute)"

        return None

    def _tap_key(self, virtual_key: int, times: int = 1):
