...
def get_voice_logger() -> logging.Logger:
    ...
    log_path = Path(LOG_DIR) / "voice.log"
    ...
    try:
        ...
        legacy_path = legacy_dir / "voice.log"
...
def log_voice_event(event: str, data: Dict[str, Any], *, error: Optional[str] = None) -> None:
    payload = {"ts": time.time(), "event": event, "data": data, "error": error}
    try:
        get_voice_logger().info(json.dumps(payload, ensure_ascii=False))
    except Exception:
        return
