...
_LOGGER_NAME = "reasoning"
_configured = False


def get_reasoning_logger() -> logging.Logger:
    global _configured
    ...
    log_path = Path(LOG_DIR) / "reasoning.log"
    ...
    try:
        ...
        legacy_path = legacy_dir / "reasoning.log"
...
def log_reasoning_event(event: str, data: Dict[str, Any], *, error: Optional[str] = None) -> None:
    payload = {"ts": time.time(), "event": event, "data": data, "error": error}
    try:
        get_reasoning_logger().info(json.dumps(payload, ensure_ascii=False))
    except Exception:
        return
