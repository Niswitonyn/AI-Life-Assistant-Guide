...
def get_rag_logger() -> logging.Logger:
    ...
    try:
        ...
        legacy_path = legacy_dir / "rag.log"
...
def log_rag_event(event: str, data: Dict[str, Any], *, error: Optional[str] = None) -> None:
    payload = {"ts": time.time(), "event": event, "data": data, "error": error}
    try:
        get_rag_logger().info(json.dumps(payload, ensure_ascii=False))
    except Exception:
        return
